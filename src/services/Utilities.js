...
class Utilities {
  ...
  getButtonDisplayName(button, display, mergeDisplay) {
    if (mergeDisplay) {
      display = { ...this.getDefaultDiplay(), ...display };
    } else {
  ...
  handleMaxLength(inputObj, updatedInput) {
    ...
    if (typeof maxLength === 'object') {
      const maxCondition = currentInput.length === maxLength[options.inputName];

      if (options.debug) {
        console.log('maxLength (obj) reached:', maxCondition);
      }
  ...
  camelCase(string) {
    ...
    return string
      ...
      .trim()
      .split(/[.\-_\s]/g)
      .reduce((syllable, word) =>
        word.length
          ? syllable + word[0].toUpperCase() + word.slice(1)
          : syllable,
      );
  }
}
