...
class CandidateBox {
  ...
  renderPage({
    ...
    candidateListPages[pageIndex].forEach((candidateListItem) => {
      ...
      const getMouseEvent = () => {
        const mouseEvent = new (
          this.options.useTouchEvents ? TouchEvent : MouseEvent
        )('click');
      ...
      candidateListLIElement.innerHTML =
        this.options.display?.[candidateListItem] || candidateListItem;
